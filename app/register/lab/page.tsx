...
import LabForm from "@/app/components/register/lab/lab-form";
import { Alert } from "@mui/material";

export default async function LabSelect({searchParams}: {searchParams: {userid: number, warn: string}}) {
    const warn = searchParams.warn;
    let warning
    if (warn === "true") {
        warning = "Debes elegir un laboratorio para poder iniciar sesión."
    } else {
        warning = ""
    };
    return (
        <main className="flex flex-row h-screen w-screen bg-gray-100">
            ...
            <div className="flex flex-col items-center justify-center gap-6 h-full w-full md:w-3/5">
                ...
                <div className="flex w-screen md:w-full justify-center items-center">
                    <LabForm userid={searchParams.userid}/>
                </div>
            </div>
        </main>
    );
